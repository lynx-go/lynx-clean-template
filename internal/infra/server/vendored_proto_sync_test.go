...
// TestVendoredAuthzProtoInSyncWithLibrary（grpcapi 接入守卫）：vendored 的
...
func TestVendoredAuthzProtoInSyncWithLibrary(t *testing.T) {
	// go test 的 cwd 是包目录（internal/infra/server）：3 级上溯到模板仓库
	// 根，4 级到父目录（grpcapi 与模板同级检出）。
	...
	if err != nil {
		t.Fatalf("vendored proto 读取失败 %s: %v", vendored, err)
	}
	libraryBytes, err := os.ReadFile(library)
	if err != nil {
		t.Fatalf("库 proto 读取失败 %s: %v", library, err)
	}

	if !bytes.Equal(vendoredBytes, libraryBytes) {
		t.Fatalf("vendored proto 与 grpcapi 库本体漂移：\n  vendored: %s\n  library:  %s\n请同步 proto/third_party/grpcapi/v1/authz.proto（随库版本升级同 PR）",
			vendored, library)
	}
}
